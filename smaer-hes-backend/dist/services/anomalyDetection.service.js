...
class AnomalyDetectionService {
    ...
    static async detectAnomalies() {
        try {
            ...
            const meters = await Meter_model_1.Meter.find({
                isActive: true,
    ...
    static async detectZeroConsumption(meter) {
        try {
            const twoDaysAgo = (0, moment_1.default)().subtract(48, 'hours').toDate();
            const recentConsumption = await Consumption_model_1.Consumption.find({
                meter: meter._id,
            ...
            if (allZero) {
                ...
                const existingAlert = await Alert_model_1.Alert.findOne({
                    meter: meter._id,
                    alertType: Alert_model_1.ALERT_TYPES.ANOMALY_ZERO_CONSUMPTION,
    ...
    static async detectConsumptionDrop(meter) {
        try {
            const today = (0, moment_1.default)().startOf('day');
            const yesterday = (0, moment_1.default)().subtract(1, 'day').startOf('day');
            const lastMonth = (0, moment_1.default)().subtract(30, 'days').startOf('day');
            // Get yesterday's consumption
            const yesterdayConsumption = await Consumption_model_1.Consumption.findOne({
                meter: meter._id,
                timestamp: {
            ...
            const lastMonthConsumption = await Consumption_model_1.Consumption.aggregate([
                {
                    $match: {
                        meter: meter._id,
                        timestamp: {
            ...
            if (yesterdayConsumption && lastMonthConsumption.length > 0) {
                ...
                if (avgLastMonth > 0) {
                    ...
                    if (dropPercentage > 80) {
                        ...
                        const existingAlert = await Alert_model_1.Alert.findOne({
                            meter: meter._id,
                            alertType: Alert_model_1.ALERT_TYPES.ANOMALY_CONSUMPTION_DROP,
                            status: { $in: ['active', 'acknowledged'] },
    ...
    static async detectNeighborhoodVariance(meter) {
        try {
            if (!meter.area)
                return;
            const yesterday = (0, moment_1.default)().subtract(1, 'day').startOf('day');
            ...
            const areaMeters = await Meter_model_1.Meter.find({
                area: meter.area._id,
                _id: { $ne: meter._id },
                isActive: true
            });
            if (areaMeters.length < 3)
                return; // Need at least 3 other meters for comparison
            // Get yesterday's consumption for all area meters
            const consumptions = await Consumption_model_1.Consumption.find({
                meter: { $in: [...areaMeters.map((m) => m._id), meter._id] },
                timestamp: {
                    $gte: yesterday.toDate(),
                    $lt: today.toDate()
                },
            ...
            if (avgAreaConsumption > 0) {
                ...
                if (variancePercentage > 70) { // Consumption is 70% less than area average
                    ...
                    const existingAlert = await Alert_model_1.Alert.findOne({
                        meter: meter._id,
                        alertType: Alert_model_1.ALERT_TYPES.ANOMALY_NEIGHBORHOOD_VARIANCE,
                        status: { $in: ['active', 'acknowledged'] },
                        triggeredAt: { $gte: yesterday.toDate() }
                    });
                    if (!existingAlert) {
    ...
    static async detectUnusualPattern(meter) {
        try {
            const lastWeek = (0, moment_1.default)().subtract(7, 'days').startOf('day');
            // Get hourly consumption for the last week
            const hourlyConsumption = await Consumption_model_1.Consumption.find({
                meter: meter._id,
                timestamp: { $gte: lastWeek.toDate() },
                interval: 'hourly'
            }).sort('timestamp');
            if (hourlyConsumption.length < 24)
                return; // Need at least a day's worth of data
            ...
            if (noVariation || hasNegativeValues) {
                const existingAlert = await Alert_model_1.Alert.findOne({
                    meter: meter._id,
                    alertType: Alert_model_1.ALERT_TYPES.ANOMALY_UNUSUAL_PATTERN,
                    status: { $in: ['active', 'acknowledged'] },
...
//# sourceMappingURL=anomalyDetection.service.js.map
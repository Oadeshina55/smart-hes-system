...
exports.DLMSData = exports.COSEMApdu = exports.HDLCFrame = exports.DLMS_CONSTANTS = void 0;
exports.parseObisCode = parseObisCode;
exports.obisToBytes = obisToBytes;
exports.obisToString = obisToString;
...
function parseObisCode(obisString) {
...
class HDLCFrame {
    ...
    build() {
        ...
        // Control field
        frame.push(Buffer.from([exports.DLMS_CONSTANTS.HDLC_I]));
...
// COSEM APDU Builder
class COSEMApdu {
    static buildAARQ(clientId = 16, password) {
        // Build Association Request (AARQ)
        const apdu = [];
        apdu.push(Buffer.from([exports.DLMS_CONSTANTS.TAG_AARQ]));
    ...
    static buildGetRequest(obisCode, classId, attributeId) {
        const apdu = [];
        apdu.push(Buffer.from([
            exports.DLMS_CONSTANTS.TAG_GET_REQUEST,
            exports.DLMS_CONSTANTS.GET_REQUEST_NORMAL,
            0x00, // Invoke ID
        ]));
    ...
    static buildSetRequest(obisCode, classId, attributeId, value) {
        const apdu = [];
        apdu.push(Buffer.from([
            exports.DLMS_CONSTANTS.TAG_SET_REQUEST,
            exports.DLMS_CONSTANTS.SET_REQUEST_NORMAL,
            0x00, // Invoke ID
        ]));
        // COSEM Attribute Descriptor
        apdu.push(Buffer.from([
    ...
    static buildActionRequest(obisCode, classId, methodId, parameters) {
        const apdu = [];
        apdu.push(Buffer.from([
            exports.DLMS_CONSTANTS.TAG_ACTION_REQUEST,
            exports.DLMS_CONSTANTS.ACTION_REQUEST_NORMAL,
            0x00, // Invoke ID
        ]));
    ...
    static buildRLRQ() {
        ...
        return Buffer.from([
            exports.DLMS_CONSTANTS.TAG_RLRQ,
            0x03,
            0x80, 0x01, 0x00, // Reason: normal
        ]);
    }
}
exports.COSEMApdu = COSEMApdu;
// Data Encoding Utilities
class DLMSData {
    static encodeUnsigned(value) {
        return Buffer.from([
            exports.DLMS_CONSTANTS.DATA_TYPE_UNSIGNED,
            value & 0xFF,
        ]);
    }
    static encodeLongUnsigned(value) {
        return Buffer.from([
            exports.DLMS_CONSTANTS.DATA_TYPE_LONG_UNSIGNED,
            (value >> 8) & 0xFF,
            value & 0xFF,
        ]);
    }
    static encodeDoubleLongUnsigned(value) {
        return Buffer.from([
            exports.DLMS_CONSTANTS.DATA_TYPE_DOUBLE_LONG_UNSIGNED,
            (value >> 24) & 0xFF,
            (value >> 16) & 0xFF,
            (value >> 8) & 0xFF,
            value & 0xFF,
    ...
    static encodeOctetString(data) {
        const buffer = typeof data === 'string' ? Buffer.from(data, 'ascii') : data;
        const result = Buffer.alloc(buffer.length + 2);
        result[0] = exports.DLMS_CONSTANTS.DATA_TYPE_OCTET_STRING;
        result[1] = buffer.length;
        buffer.copy(result, 2);
        return result;
    }
    static encodeVisibleString(text) {
        const buffer = Buffer.from(text, 'ascii');
        const result = Buffer.alloc(buffer.length + 2);
        result[0] = exports.DLMS_CONSTANTS.DATA_TYPE_VISIBLE_STRING;
        result[1] = buffer.length;
        buffer.copy(result, 2);
        return result;
    }
    static encodeDateTime(date) {
        const result = Buffer.alloc(14);
        result[0] = exports.DLMS_CONSTANTS.DATA_TYPE_DATETIME;
        result[1] = 12; // Length
    ...
    static decodeData(buffer, offset = 0) {
        const dataType = buffer[offset];
        let value;
        let length = 1;
        switch (dataType) {
            case exports.DLMS_CONSTANTS.DATA_TYPE_BOOLEAN:
                value = buffer[offset + 1] !== 0;
                length = 2;
                break;
            case exports.DLMS_CONSTANTS.DATA_TYPE_UNSIGNED:
                value = buffer[offset + 1];
                length = 2;
                break;
            case exports.DLMS_CONSTANTS.DATA_TYPE_LONG_UNSIGNED:
                value = (buffer[offset + 1] << 8) | buffer[offset + 2];
                length = 3;
                break;
            case exports.DLMS_CONSTANTS.DATA_TYPE_DOUBLE_LONG_UNSIGNED:
                value = (buffer[offset + 1] << 24) | (buffer[offset + 2] << 16) |
                    (buffer[offset + 3] << 8) | buffer[offset + 4];
                length = 5;
                break;
            case exports.DLMS_CONSTANTS.DATA_TYPE_OCTET_STRING:
            case exports.DLMS_CONSTANTS.DATA_TYPE_VISIBLE_STRING:
                const strLength = buffer[offset + 1];
                value = buffer.slice(offset + 2, offset + 2 + strLength);
                if (dataType === exports.DLMS_CONSTANTS.DATA_TYPE_VISIBLE_STRING) {
            ...
            default:
                value = null;
                logger_1.default.warn(`Unknown DLMS data type: 0x${dataType.toString(16)}`);
        }
        return { value, length };
    }
}
exports.DLMSData = DLMSData;
exports.default = {
    DLMS_CONSTANTS: exports.DLMS_CONSTANTS,
    parseObisCode,
    obisToBytes,
    obisToString,
    HDLCFrame,
...
//# sourceMappingURL=dlmsProtocol.js.map
 ...
 */
export function parseHexcellObisList(filePath: string): EnhancedObisCode[] {
  const content = fs.readFileSync(filePath, 'utf-8');
  const lines = content.split('\n');
  const obisCodes: EnhancedObisCode[] = [];
      ...
      if (attributeIdStr) {
        ...
        for (const part of attrParts) {
          if (part.includes(':')) {
            ...
            if (access?.includes('R')) canRead = true;
            if (access?.includes('W')) canWrite = true;
          }
        }
      }
 ...
 */
export function parseHexingObisList(filePath: string): EnhancedObisCode[] {
  const content = fs.readFileSync(filePath, 'utf-8');
  const lines = content.split('\n');
  const obisCodes: EnhancedObisCode[] = [];
 ...
 */
function convertObisCodeFormat(hexCode: string): string {
  if (hexCode.length !== 12) return hexCode;
 ...
 */
export function parseDLMSConfig(filePath: string): any {
  const content = fs.readFileSync(filePath, 'utf-8');
 ...
 */
export function loadSoftwareConfigurations(): {
  ...
  let hexcellCodes: EnhancedObisCode[] = [];
  let hexingCodes: EnhancedObisCode[] = [];
 ...
 */
export function getMeterModelConfiguration(brand: 'hexing' | 'hexcell', model?: string): MeterConfiguration {
  const configs = loadSoftwareConfigurations();
  const obisCodes = brand === 'hexing' ? configs.hexing : configs.hexcell;

  let connectionParams: any = undefined;

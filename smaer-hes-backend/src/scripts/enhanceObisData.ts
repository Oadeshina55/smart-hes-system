...
import {
  parseHexcellObisList,
  parseHexingObisList,
  EnhancedObisCode,
} from '../utils/softwareConfigParser';

interface ExistingObisFunction {
  code: string;
  name: string;
  description: string;
  unit?: string;
 ...
 */
function mergeObisData(
  existing: ExistingObisFunction[],
  softwareConfig: EnhancedObisCode[]
): ExistingObisFunction[] {
  const merged: ExistingObisFunction[] = [];
  const softwareMap = new Map<string, EnhancedObisCode>();
...
async function main() {
  ...
  const hexcellSoftwareConfig = fs.existsSync(hexcellConfigPath)
    ? parseHexcellObisList(hexcellConfigPath)
    : [];
  ...
  const hexingSoftwareConfig = fs.existsSync(hexingConfigPath)
    ? parseHexingObisList(hexingConfigPath)
    : [];
  ...
  let hexcellExisting: { functions: ExistingObisFunction[] } = { functions: [] };
  let hexingExisting: { functions: ExistingObisFunction[] } = { functions: [] };
  ...
  fs.writeFileSync(
    enhancedHexcellPath,
    JSON.stringify({ functions: mergedHexcell }, null, 2),
    'utf-8'
  );
  ...
  fs.writeFileSync(
    enhancedHexingPath,
    JSON.stringify({ functions: mergedHexing }, null, 2),
    'utf-8'
  );
